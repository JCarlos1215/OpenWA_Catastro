  ...
  return {
    async send(env: ConversationSendEnvelope): Promise<unknown> {
      ...
      return deps.runGuarded(MESSAGE_HOOK_EVENTS, async () => {
        // A media type carrying a URL or inline base64 payload is sent as native media. A media type without
        // either has nothing to send as media, so it falls through to the text/reply path — a plugin that puts the
        // URL in `text` as a fallback still delivers a (text) message rather than erroring.
        if (isMediaType(env.type) && (env.mediaUrl || env.mediaBase64)) {
          ...
          return deps.sendMedia(sessionId, {
            chatId,
            ...(env.mediaUrl ? { url: env.mediaUrl } : {}),
            ...(env.mediaBase64 ? { base64: env.mediaBase64 } : {}),
            ...(env.mimetype ? { mimetype: env.mimetype } : {}),
            ...(env.filename ? { filename: env.filename } : {}),
            type: env.type,
            caption: env.text,
          });
        }
