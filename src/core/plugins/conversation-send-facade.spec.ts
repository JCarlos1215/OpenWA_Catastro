...
describe('conversation.send facade', () => {
  ...
  it('routes an inline PDF payload to sendMedia with its MIME type and filename', async () => {
    const sendMedia = jest.fn().mockResolvedValue({ id: 'pdf-1' });
    const facade = buildConversationSendFacade({
      manifest: manifest(['conversation:send']) as never,
      assertPermission: () => undefined,
      assertSessionActive: jest.fn(),
    ...
    await facade.send({
      type: 'file',
      mediaBase64: 'data:application/pdf;base64,QUJD',
